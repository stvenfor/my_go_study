...
|------|---------|--------------|------|
| 登录注册 | `BackendAuthService` | `UserHandler` → `SupabaseAuthUsecase` | Supabase Auth |
...
| Token 校验 | `middleware.SupabaseAuth` | `GET /auth/v1/user` |
...
|--------|----------|------------|---------|
| `SupabaseAuth` | `/api/v1/transactions*`、`/api/v1/realtime/*` | Supabase access token | UUID |
| `Auth`（JWT） | `/api/v1/user/list` | Go 自建 JWT | `uint` |

Flutter 登录返回 **Supabase token**，业务 API 走 `SupabaseAuth`。
...
### 应用启动与依赖注入（`cmd/api/main.go`）

`make run` → `./scripts/load-env.sh go run ./cmd/api`

```text
1. APP_ENV → config.Load
2. logger.Init
3. PostgreSQL + autoMigrate
4. Redis
5. UserUsecase（遗留 JWT 路由）
...
1. 在本仓库根目录执行 `go` / `make`
2. 新增 Supabase 表：entity → repository → usecase → controller → router（`SupabaseAuth`）
